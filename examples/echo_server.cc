...
void EchoServer::handleClient(easy::Socket::ptr client)
{
    ...
    while (true)
    {
        ...
        if (rt == 0)
        {
            ELOG_INFO(logger) << "client close: " << *client;
            break;
        }
        else if (rt < 0)
        {
            ELOG_INFO(logger) << "client error rt=" << rt << " errno=" << errno << " errstr=" << strerror(errno);
            break;
        }
...
void run()
{
    ELOG_INFO(logger) << "server type=" << type;
...
int main(int argc, char** argv)
{
    if (argc < 2)
    {
        ELOG_INFO(logger) << "used as[" << argv[0] << " -t] or [" << argv[0] << " -b]";
        return 0;
    }
