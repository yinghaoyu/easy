...
Scheduler::Scheduler(int threadNums, bool use_caller, const std::string& name) : name_(name)
{
    ...
    if (use_caller)
    {
        ...
        // stacksize > 0
        callerFiber_.reset(NewFiber(std::bind(&Scheduler::run, this), 0, true), FreeFiber);
    ...
    ELOG_DEBUG(logger) << "Scheduler ctor name=" << name_;
}

Scheduler::~Scheduler()
{
    ELOG_DEBUG(logger) << "Scheduler dtor~ name=" << name_;
...
void Scheduler::start()
{
    ...
    {
        threads_[i] = std::make_shared<Thread>(std::bind(&Scheduler::run, this), name_ + "_" + std::to_string(i));
...
void Scheduler::stop()
{
    if (callerFiber_ && threadNums_ == 0 && (callerFiber_->finish() || callerFiber_->state() == Fiber::INIT))
    {
...
void Scheduler::handleFiber(Fiber::ptr& fiber)
{
    ...
    switch (fiber->state_)
    {
        case Fiber::INIT:
        case Fiber::EXEC:
        case Fiber::HOLD: fiber->state_ = Fiber::HOLD; break;
        case Fiber::READY: schedule(fiber); break;
        case Fiber::TERM:
        case Fiber::EXCEPT: break;
    }
}
...
Scheduler::Task::ptr Scheduler::take()
{
    ...
    {
        auto task = *cur;
        if (task->threadId_ != -1 && task->threadId_ != Thread::GetCurrentThreadId())
        {
            // designate thread
            continue;
        }
