...
void MutexLock::lock() { EASY_CHECK(pthread_mutex_lock(&mutex_)); }
...
SpinLock::~SpinLock() { EASY_CHECK(pthread_spin_destroy(&mutex_)); }

void SpinLock::lock() { EASY_CHECK(pthread_spin_lock(&mutex_)); }
...
void Semaphore::wait() { EASY_CHECK(sem_wait(&semaphore_)); }

void Semaphore::notify() { EASY_CHECK(sem_post(&semaphore_)); }
