...
static ConfigVar<uint32_t>::ptr fiber_stack_size = Config::Lookup<uint32_t>(
...
Fiber* NewFiber(std::function<void()> cb, size_t stacksize, bool use_caller) {
  ...
  Fiber* p =
      static_cast<Fiber*>(StackAllocator::Alloc(sizeof(Fiber) + stacksize));
...
Fiber::Fiber() {
  ...
  EASY_LOG_DEBUG(logger) << "Fiber ctor root id=" << id_;
}
...
Fiber::Fiber(std::function<void()> cb, size_t stacksize, bool use_caller)
  ...
  EASY_LOG_DEBUG(logger) << "Fiber ctor sub id=" << id_;
}
...
Fiber::~Fiber() {
  ...
  EASY_LOG_DEBUG(logger) << "Fiber dtor~ id=" << id_
...
void Fiber::YieldToHold() {
  ...
  if (Scheduler::GetThis() &&
      Scheduler::GetSchedulerFiber() != t_running_fiber) {
    cur->sched_yield();
  } else {
    cur->yield();
  }
...
void Fiber::MainFunc() {
  ...
  try {
    ...
    cur->state_ = EXCEPT;
    EASY_LOG_ERROR(logger) << "Fiber Except: " << ex.what()
  ...
  EASY_ASSERT_MESSAGE(
      false, "Never reach here, fiberId=" + std::to_string(raw_ptr->id()));
}
