...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host, int family, int type, int protocol)
{
    ...
    if (node.empty())
    {
        ...
        if (service)
        {
            if (!memchr(service + 1, ':', static_cast<size_t>(host.c_str() + host.size() - service - 1)))
            {
    ...
    if (error)
    {
        ELOG_ERROR(logger) << "Address::Lookup getaddress(" << host << ", " << family << ", " << type << ") err=" << error
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>>& result, int family)
{
    ...
    {
        ELOG_ERROR(logger) << "Address::GetInterfaceAddresses getifaddrs "
                              " err="
    ...
    {
        ELOG_ERROR(logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
Address::ptr Address::Create(const sockaddr* addr, socklen_t addrlen)
{
    ...
    {
        case AF_INET: result = std::make_shared<IPv4Address>(*reinterpret_cast<const sockaddr_in*>(addr)); break;
        case AF_INET6: result = std::make_shared<IPv6Address>(*reinterpret_cast<const sockaddr_in6*>(addr)); break;
...
bool Address::operator==(const Address& other) const { return addrLen() == other.addrLen() && memcmp(addr(), other.addr(), addrLen()) == 0; }
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port)
{
    ...
    if (error)
    {
        ELOG_DEBUG(logger) << "IPAddress::Create(" << address << ", " << port << ") error=" << error << " errno=" << errno
                           << " errstr=" << strerror(errno);
        return nullptr;
    }
    try
    {
        IPAddress::ptr result = std::dynamic_pointer_cast<IPAddress>(Address::Create(results->ai_addr, static_cast<socklen_t>(results->ai_addrlen)));
        if (result)
        {
            result->setPort(port);
        }
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port)
{
    ...
    if (result <= 0)
    {
        ELOG_DEBUG(logger) << "IPv4Address::Create(" << address << ", " << port << ") rt=" << result << " errno=" << errno
                           << " errstr=" << strerror(errno);
        return nullptr;
    }
    return rt;
...
sockaddr* IPv4Address::addr() { return reinterpret_cast<sockaddr*>(&addr_); }

const sockaddr* IPv4Address::addr() const { return reinterpret_cast<const sockaddr*>(&addr_); }
...
uint32_t IPv4Address::port() const { return networkToHost(addr_.sin_port); }

void IPv4Address::setPort(uint16_t v) { addr_.sin_port = hostToNetwork(v); }
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port)
{
    ...
    if (result <= 0)
    {
        ELOG_DEBUG(logger) << "IPv6Address::Create(" << address << ", " << port << ") rt=" << result << " errno=" << errno
                           << " errstr=" << strerror(errno);
        return nullptr;
    }
    return rt;
...
sockaddr* IPv6Address::addr() { return reinterpret_cast<sockaddr*>(&addr_); }

const sockaddr* IPv6Address::addr() const { return reinterpret_cast<const sockaddr*>(&addr_); }
...
{
    os << "[";
    const uint16_t* addr       = reinterpret_cast<const uint16_t*>(addr_.sin6_addr.s6_addr);
...
IPAddress::ptr IPv6Address::subnetMask(uint32_t prefix_len)
{
    ...
    subnet.sin6_addr.s6_addr[prefix_len / 8] = static_cast<uint8_t>(~CreateMask<uint8_t>(prefix_len % 8));
...
uint32_t IPv6Address::port() const { return networkToHost(addr_.sin6_port); }

void IPv6Address::setPort(uint16_t v) { addr_.sin6_port = hostToNetwork(v); }

static const size_t MAX_PATH_LEN = sizeof((static_cast<sockaddr_un*>(0))->sun_path) - 1;
...
sockaddr* UnixAddress::addr() { return reinterpret_cast<sockaddr*>(&addr_); }

const sockaddr* UnixAddress::addr() const { return reinterpret_cast<const sockaddr*>(&addr_); }

socklen_t UnixAddress::addrLen() const { return length_; }
...
sockaddr* UnknownAddress::addr() { return reinterpret_cast<sockaddr*>(&addr_); }

const sockaddr* UnknownAddress::addr() const { return reinterpret_cast<const sockaddr*>(&addr_); }
