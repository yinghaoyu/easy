...
namespace easy {
static ConfigVar<uint64_t>::ptr tcp_server_read_timeout = Config::Lookup(
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs,
  ...
  for (auto& i : socks_) {
    EASY_LOG_INFO(logger) << "type=" << type_ << " name=" << name_
                          << " ssl=" << ssl_ << " server bind success: " << *i;
  }
  return true;
}
...
std::string TcpServer::toString(const std::string& prefix) {
  ...
  ss << prefix << "[type=" << type_ << " name=" << name_ << " ssl=" << ssl_
     << " worker=" << (worker_ ? worker_->name() : "")
     << " accept=" << (accept_worker_ ? accept_worker_->name() : "")
